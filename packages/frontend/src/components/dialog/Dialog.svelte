...
<Modal name={title} onclose={onclose}>
  <div class="flex items-center justify-between pl-4 pr-3 py-3 space-x-2 text-(--pd-modal-header-text)">
    {@render icon?.()}
  ...
  <div class="relative max-h-80 overflow-auto text-(--pd-modal-text) px-10 py-4">
    {@render content?.()}
  </div>
